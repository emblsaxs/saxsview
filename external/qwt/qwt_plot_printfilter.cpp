...
void QwtPlotPrintFilter::apply(QwtPlot *plot) const
{
    ...
    if ( plot->titleLabel() )
    {
       QwtText plotTitle = plot->titleLabel()->text();
       if ( plotTitle.testPaintAttribute(QwtText::PaintUsingTextColor) )
         {
             cache.titleColor = plotTitle.color();
             plotTitle.setColor(color(cache.titleColor, Title));
         }

       if ( plotTitle.testPaintAttribute(QwtText::PaintUsingTextFont) )
         {
             cache.titleFont = plotTitle.font();
             plotTitle.setFont(font(cache.titleFont, Title));
         }

        plot->titleLabel()->setText(plotTitle);
    }
